---
title: "iRIGS local R implementation"
author: "Colleen Xu"
...
This code is designed to be relatively simple, without much vectorizing/optimization with packages.   
...
```{r arguments}
arguments <- c(here("SNP_file", "SCZ_108_loci"),   ## SNP_file
               1000000,    ## flank
               here("iRIGS_results"),    ## path for results file directory
               here("iRIGS_results", "SCZ_trial1_original.txt"),   ## path for original result file
               here("iRIGS_results", "SCZ_trial1_supple1.txt"))    ## path for supplementary table 1 file
```
...
# length(unique(colnames(pro_p)))  ## shows that all gene symbols in the network are unique
...
## find rows of gene information with the same official_name values that are in the gene network
ex0 <- gene_info %>% 
...
## can also check the other row sums: row 21, 206, 558, etc. 
```
...
rownames(network_matrix) <- colnames(network_matrix)  ## somehow rownames got removed...add them back
...
After [looking through](http://grch37.ensembl.org/Homo_sapiens/Info/Index)  the Name column (ENSEMBL IDs), I found that some 'duplicate' alias symbols were different transcripts - where one was matched the Ensembl version. I went through those manually to create the array of IDs that matched the Ensembl version.   
...
The final gene_info table only has genes in the connected part of the gene-gene network. There are 17035 rows (unique Ensembl IDs in the 'Name' column) and 17017 unique gene symbols (alias_symbol column). The duplicate gene symbols are from 18 pseudoautosomal genes.    
...
Note: the Mahalanobis transformation is supposed to create variables that follow the univariable Gaussian (normal) distribution but I feel like it doesn't given the output of skim
```{r switchsign}
...
Takes me ~ 25 min to run, ~ 606 runs thru 101 loci with > 1 candidate gene . 
...
# set.seed(431)  ## to set seed during coding
## note that sample_n works only for local tables: ref https://dplyr.tidyverse.org/reference/sample.html
current_geneset <- candidate_genes %>% 
  group_by(SNP) %>%   ## notice that R automatically sorts table by alphabetical order of this key
  sample_n(1) %>% 
  select(SNP, alias_symbol)
...
{
  ## store Gibbs sample sets for the current round
  ...
  {  ## note that if there is only one candidate gene for the SNP loci, the chunk doesn't run and the loci is effectively SKIPPED for Gibbs sampling
    ...
    { 
      ...
      {  
        ...
        ##      I haven't had this flag go off before. Would probably have to be dealt with 
        ##       by drawing a new current_geneset sample (like what is done at the beginning). 
        {
...
**Remember to change the paths/file names for results between runs if you don't want to rewrite your files from previous runs.**   
